		...
		try {
			...
			while(true) {
				...
				System.out.println(
					...
					+socket.getPort()+" 포트를통해"//랜덤숫자
					+socket.getLocalAddress()+"(서버)의"
					+socket.getLocalPort()+"포트로 연결됨");//9999
...
//						(PrintWriter)clientMap.get(it.next()); //기존거
...
//							sendAllMsg(strArr[1], strArr[2], "One");//오류(띄어쓰기 이후는 전송안됨)
						}
					}
					else {
